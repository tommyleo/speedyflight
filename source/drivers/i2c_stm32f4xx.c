...
bool i2cWriteBuffer(uint8_t addr_, uint8_t reg_, uint8_t len_, uint8_t *data)
{
    ...
    if (!(I2C1->CR2 & I2C_IT_EVT)) {                                    // if we are restarting the driver
        if (!(I2C1->CR1 & 0x0100)) {                                    // ensure sending a start
            while (I2C1->CR1 & 0x0200 && --timeout > 0) { ; }           // wait for any stop to finish sending
            if (timeout == 0)
...
static void i2c_er_handler(void)
{
    ...
    volatile uint32_t SR1Register = I2C1->SR1;
    ...
    if (SR1Register & 0x0700) {
        (void)I2C1->SR2;                                                // read second status register to clear ADDR if it is set (note that BTF will not be set after a NACK)
        I2C_ITConfig(I2C1, I2C_IT_BUF, DISABLE);                        // disable the RXNE/TXE interrupt - prevent the ISR tailchaining onto the ER (hopefully)
        if (!(SR1Register & 0x0200) && !(I2C1->CR1 & 0x0200)) {         // if we dont have an ARLO error, ensure sending of a stop
            if (I2C1->CR1 & 0x0100) {                                   // We are currently trying to send a start, this is very bad as start, stop will hang the peripheral
                ...
                i2cInit(I2C1);                                    // reset and configure the hardware
            } else {
